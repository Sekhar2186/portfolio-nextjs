    ...
    <nav className="fixed top-0 left-0 right-0 h-[72px] z-50 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-xl border-b border-gold-subtle">
      <div className="max-w-[1200px] mx-auto h-full px-6 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center group">
          <span className="text-gold font-bold text-xl transition-all group-hover:drop-shadow-[0_0_8px_rgba(212,175,55,0.5)]">AI</span>
          <span className="text-background-dark dark:text-white font-normal text-xl">with Surya</span>
        </Link>
          ...
          {[
            ...
            <Link
              ...
              className={`text-sm font-medium transition-colors hover:text-gold ${
                isActive(link.path) ? 'text-gold' : 'text-background-dark/80 dark:text-white/80'
              }`}
            >
              {link.name}
            </Link>
